...
class AiRepository {
    suspend fun hikeyChatCompletion(messageList: List<Message>?): AiResponse = withContext(Dispatchers.IO) {
        ...
        val request = AiRequest(
            ...
            ) + (messageList?.toList() ?: listOf())
        )
        Log.d("Connect ChatGPT", "2. Ai Repository")
        return@withContext RetrofitInstance.aiApi.createChatCompletion(request)
    }

    suspend fun bongbongChatCompletion(messageList: List<Message>?): AiResponse = withContext(Dispatchers.IO) {
